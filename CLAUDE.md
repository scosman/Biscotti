...
- **`specs/projects/apple_on_device_llm/`** — a **BLOCKED** spec project (Project 16): adding an "Apple Intelligence" option alongside the Gemma models via `FoundationModels`. The API is compatible with our `LLMSession` interface, but the on-device context window (4096 tokens on macOS 26.0–26.3, 8192 on the current generation) fits only ~20 minutes of meeting after our prompt and output reserves. `research.md` carries the full API survey and token math; `functional_spec.md` is complete and shelf-ready. **Don't re-derive these findings** — read them before any future Foundation Models work. Unblocked by Project 15 (Long-Transcript Analysis), a larger Apple context window, or a product decision to accept short meetings only.
