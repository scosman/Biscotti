...
/// real-time thread. Each slot has a fixed-size buffer sized for
/// `maxFrameCount` float samples. Buffers exceeding this size are dropped
/// and counted.
///
/// Carried over from `experiments/AudioLab/Sources/AudioRingBuffer.swift`.
final class AudioRingBuffer: @unchecked Sendable {
    ...
    ///   - maxFrameCount: Maximum float samples per buffer. Buffers larger
    ///     than this are dropped (counted via `droppedBuffers`). Default 8192
    ///     covers typical IOProc callbacks (1024-4096 frames) with headroom.
    init(capacity: Int = 512, maxFrameCount: Int = 8192) {
        self.capacity = capacity
        slotByteSize = maxFrameCount * MemoryLayout<Float>.size
    ...
    /// Called from the writer thread. Returns nil if empty. The returned
    /// Entry's `data` pointer is valid only until the next `dequeue()` call
