...
struct RouteChangeTests {
    ...
    @Test("Input change reconnects mic capture without stop/start (file-preserving)")
    func inputChangeReconnectsMicCapture() async throws {
        let ctx = try TestRecorderFactory.make()
        defer { TestRecorderFactory.cleanup(ctx) }
        ...
        // Mic engine should have been reconnected, NOT stopped and restarted.
        // reconnect() preserves the audio file; stop()+start() would erase it.
        #expect(ctx.micEngine.reconnectCount == 1)
        #expect(ctx.micEngine.stopCount == 0, "stop() must NOT be called on input change (would erase file)")
        #expect(ctx.micEngine.startCount == 1, "start() must NOT be called on input change (would erase file)")
    ...
    func multipleRouteChanges() async throws {
        ...
        // Send an input change (reconnects mic engine).
        ctx.deviceChangeProvider.send(.inputChanged)
        ...
        // Mic engine was reconnected once (for input change).
        #expect(ctx.micEngine.reconnectCount == 1)
        #expect(ctx.micEngine.stopCount == 0)
        #expect(ctx.micEngine.startCount == 1)
    ...
    // MARK: - Mic reconnect retry

    @Test("Transient mic reconnect failure retries and succeeds")
    func transientMicReconnectFailureRetries() async throws {
        let ctx = try TestRecorderFactory.make()
        defer { TestRecorderFactory.cleanup(ctx) }
        ...
        // First reconnect will fail, second will succeed.
        ctx.micEngine.setTransientReconnectError(
            CaptureError.micEngineFailed("transient"), failureCount: 1
        )

        ctx.deviceChangeProvider.send(.inputChanged)
        // Worst-case retry wall time is ~600ms (1 failure + 300ms delay + 1 success).
        // 1s is generous to avoid CI flakiness on loaded runners.
        try await Task.sleep(for: .seconds(1))

        // Should have retried: 1 failure + 1 success = 2 reconnect calls.
        #expect(ctx.micEngine.reconnectCount == 2)
    ...
    @Test("Permanent mic reconnect failure exhausts retries — system continues")
    func permanentMicReconnectFailureExhaustsRetries() async throws {
        let ctx = try TestRecorderFactory.make()
        defer { TestRecorderFactory.cleanup(ctx) }
        ...
        // All reconnect attempts will fail.
        ctx.micEngine.setReconnectError(CaptureError.micEngineFailed("permanent"))

        ctx.deviceChangeProvider.send(.inputChanged)
        try await Task.sleep(for: .milliseconds(1500))

        // Should have tried 1 + maxRetries = 3 total reconnect calls.
        let expectedAttempts = AudioRecorder.micReconnectMaxRetries + 1
        #expect(ctx.micEngine.reconnectCount == expectedAttempts)

        // System engine was not touched.
        #expect(ctx.systemEngine.stopCount == 0)
        #expect(ctx.systemEngine.reconnectCount == 0)
    ...
    func reconnectDistinguishedFromStart() async throws {
        ...
        // Output route change: system reconnect called, NOT start.
        ctx.deviceChangeProvider.send(.outputChanged)
        try await Task.sleep(for: .milliseconds(200))
        ...
        // Input route change: mic reconnect called, NOT start.
        ctx.deviceChangeProvider.send(.inputChanged)
        try await Task.sleep(for: .milliseconds(200))

        #expect(ctx.micEngine.startCount == 1, "start() creates/erases the file -- must only be called once")
        #expect(ctx.micEngine.reconnectCount == 1, "reconnect() preserves the file")

        // Second output route change.
        ctx.deviceChangeProvider.send(.outputChanged)
        try await Task.sleep(for: .milliseconds(200))
