...
struct TranscriptContentMergedSpeakerColorTests {
    ...
    @Test("attributed string uses color keys for merged speakers")
    func attributedStringUsesMergedColors() {
        let personID = UUID()
        let colorKeys: [Int: String] = [
            0: "person-\(personID.uuidString)",
            1: "person-\(personID.uuidString)"
        ]
        ...
        // Extract foreground colors from the speaker-label runs
        var speakerColors: [SwiftUI.Color] = []
        for run in attributed.runs {
            let text = String(attributed.characters[run.range])
            if text == "Alice", let color = run.foregroundColor {
                speakerColors.append(color)
            }
        }

        #expect(speakerColors.count == 2)
        #expect(speakerColors[0] == speakerColors[1])
    }
}
