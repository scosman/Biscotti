...
///
/// Used for unit tests (with `MockEngine`), the CLI, and as the sole current
/// backend. The `id` parameter on generate/cancel is ignored -- serialization
/// is handled by `LLMConnection`'s semaphore, so at most one request is in-flight.
final class InProcessBackend: ServiceBackend, @unchecked Sendable {
    ...
    ///
    /// The model URL and config are known at construction time but the expensive
    /// model load is deferred to `start()` (after the connection is opened).
