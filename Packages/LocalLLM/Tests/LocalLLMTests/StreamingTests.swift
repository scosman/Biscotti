...
struct StreamingTests {
    ...
    func tokenEquality() {
        let event1 = StreamEvent.token("hello")
        let event2 = StreamEvent.token("hello")
        let event3 = StreamEvent.token("world")
    ...
    func reasoningTokenEquality() {
        let event1 = StreamEvent.reasoningToken("think")
        let event2 = StreamEvent.reasoningToken("think")
        let event3 = StreamEvent.reasoningToken("other")
        #expect(event1 == event2)
        #expect(event1 != event3)
    }
    ...
    func doneEquality() {
        ...
        let event1 = StreamEvent.done(result)
        let event2 = StreamEvent.done(result)
        #expect(event1 == event2)
    }
...
struct StreamingChannelSplitterTests {
    ...
    func markersNeverLeakIntoOutput() {
        ...
        for piece in allPieces {
            ...
            case let .content(str): str
            case let .reasoning(str): str
            }
