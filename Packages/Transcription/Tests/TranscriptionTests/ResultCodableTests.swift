...
// MARK: - XPCProcessRequest Codable

@Suite("XPCProcessRequest Codable")
struct XPCProcessRequestCodableTests {
    @Test("Round-trips with diarizationClusterThreshold set")
    func roundTripWithThreshold() throws {
        let request = XPCProcessRequest(
            micPath: "/tmp/mic.aac",
            systemPath: "/tmp/system.aac",
            customVocabulary: ["Biscotti", "WhisperKit"],
            diarizationClusterThreshold: 0.35
        )

        let data = try JSONEncoder().encode(request)
        let decoded = try JSONDecoder().decode(XPCProcessRequest.self, from: data)

        #expect(decoded.micPath == "/tmp/mic.aac")
        #expect(decoded.systemPath == "/tmp/system.aac")
        #expect(decoded.customVocabulary == ["Biscotti", "WhisperKit"])
        #expect(decoded.diarizationClusterThreshold == 0.35)
    }

    @Test("Round-trips with diarizationClusterThreshold nil")
    func roundTripWithNilThreshold() throws {
        let request = XPCProcessRequest(
            micPath: "/tmp/mic.aac",
            systemPath: "/tmp/system.aac",
            customVocabulary: []
        ...
        #expect(decoded.micPath == "/tmp/mic.aac")
        #expect(decoded.systemPath == "/tmp/system.aac")
        #expect(decoded.customVocabulary.isEmpty)
        #expect(decoded.diarizationClusterThreshold == nil)
    }

    @Test("Decodes JSON without diarizationClusterThreshold field (backward compatibility)")
    func backwardCompatibility() throws {
        // Simulate a payload from an older client that lacks the field
        let json = """
        {
            "micPath": "/tmp/mic.aac",
            "systemPath": "/tmp/system.aac",
        ...
        #expect(decoded.micPath == "/tmp/mic.aac")
        #expect(decoded.systemPath == "/tmp/system.aac")
        #expect(decoded.customVocabulary == ["test"])
        #expect(decoded.diarizationClusterThreshold == nil)
    }
}
