...
struct ArgMaxCLI: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        guard FileManager.default.fileExists(atPath: fileURL.path) else {
            printErr("Error: File not found: \(audioFile)")
        ...
        printErr("ArgMaxKit CLI")
        printErr("=============")
        printErr("Audio file: \(fileURL.path)")
        printErr("STT model:  \(model)")
        if !vocabTerms.isEmpty {
            printErr("Vocabulary: \(vocabTerms.joined(separator: ", "))")
        }
        printErr("Sequential: \(sequential)")
        printErr()

        let processor = ArgMaxProcessor(config: config)

        printErr("Loading models and processing audio...")
        printErr("(First run will download models and compile CoreML — this may take several minutes)")
        printErr()
    ...
    private func printJSON(_ result: TranscriptResult) {
        ...
        guard let data = try? encoder.encode(result),
            ...
            printErr("Error: Failed to encode result as JSON")
            return
        }
        print(jsonString)
    }
