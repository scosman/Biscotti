...
- [ ] **Phase 3 — Consumers, docs, manual test.** The `app_url` field on
  `biscotti_get_meeting` (+ schema and fixture updates), `App/deeplinks.md`,
  and the `app_urls` ManualTestKit script with its `WiredScripts` wiring.
  Marks `mcp_*` manual steps `not-run` per the staleness rule.
  Architecture §6–§8.
