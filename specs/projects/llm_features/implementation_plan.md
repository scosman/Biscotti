...
- [ ] **Phase 7 — End-to-end verification & docs.** On-hardware manual run (download model → record/transcribe → auto speaker-ID + streamed summary → edit/regenerate → manual rename); tune prompt wording if needed; update root `architecture.md`/`implementation_plan.md` roadmap status; optional new ManualTestKit script for the AI features. **Gate:** full `ci` + `build-app` green. — *Agent portion complete:* root roadmap/docs updated (`architecture.md`, `implementation_plan.md`, `CLAUDE.md`, `app_overview.md`), `ai_*` ManualTestKit script added (21 steps, 18 recordable), full `ci` + `build-app` green. *Deferred to human (on-hardware):* download model → record/transcribe → verify auto speaker-ID + streamed summary → edit/regenerate → manual rename → tune `IntelligencePrompts` if needed → record the 18 `ai_*` manual-test results.
