...
| **Audio format & streaming** | **ADTS AAC-LC, 64 kbps mono, 24 kHz** (`.aac` via `ExtAudioFile` + `kAudioFileAAC_ADTSType`); self-syncing → crash-safe with no finalization (no CAF, no PCM scratch, no encode-on-stop) | Two independent streams (`{id}_mic.aac` + `{id}_system.aac`), shared start timestamp. ~1 MB/min total. *(Phase 9 RESOLVED; supersedes the earlier CAF/48 kHz/M4A note.)* [Details](audio/README.md) |
...
| **Diarization model** | Pyannote v4 community-1 via SpeakerKit (free SDK, ~33 MB) | Per-file speaker clustering; centroid embeddings **NOT exposed in v1.0.0** (see argmax §6 erratum) — cross-file matching reserved for future. [Details](argmax/README.md) |
| **ML isolation** | XPC service (`BiscottiTranscriber.xpc`) with `NSXPCConnection` | Crash isolation, memory isolation, auto-relaunch by `launchd`. **Must validate XPC + CoreML in E3.** [Details](argmax/README.md) |
