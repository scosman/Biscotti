...
**⚠️ Silent total-failure mode — FIXED in v1.1.0 (hardware-validated in v1.0.0, Project A.5 — Apple silicon, macOS 15):**
In argmax-oss-swift v1.0.0, certain prompts made WhisperKit return an **entirely empty transcript** (zero segments, no error). Observed triggers: uppercase terms (e.g. `NASA`), certain term orderings/combinations even when lowercase. Root cause (confirmed in [PR #514](https://github.com/argmaxinc/argmax-oss-swift/pull/514)): the decode loop's early-exit checks (EOT during prefill, first-token confidence) honored throwaway tokens from the forced prompt, causing immediate segment termination. **Fixed in v1.1.0** — the EOT check ignores prefill steps, and the first-token confidence check is anchored at the first real decoded token.

**Mitigations (defense-in-depth, less critical now that root cause is fixed):**
- **Guard against empty output:** if a vocab-conditioned pass returns zero segments, **re-run without the prompt** — still a good safety net even with the fix.
- **Cap/curate** the term set; treat the prompt as best-effort.
- The Pro custom-vocabulary API (a separate vocabulary-matching pass, not prompt conditioning) avoids this class of issue entirely.

**Recommendation:** Use the `promptTokens` approach in the free SDK. Build the API to accept a `[String]` vocabulary list, format it into a natural-language prompt, tokenize it, and pass it as `promptTokens`. This lets us swap to Pro's custom vocabulary later with no API change for callers. See Gotcha #16. The blanking bug is **fixed in argmax-oss-swift v1.1.0** ([PR #514](https://github.com/argmaxinc/argmax-oss-swift/pull/514)) — the `make test-ai` custom-vocab test (`customVocabWordMatch`) is re-enabled and **passes on hardware** (2026-08-23). `VocabularyFormatter` preserves original term casing (the v1.0.0 lowercasing workaround was removed after on-hardware verification confirmed mixed case works with v1.1.0 and that lowercasing actually degraded accuracy — see Gotcha #16). The empty-output re-run guard (mitigation b) remains deferred — less critical now that the root cause is fixed, but still a good defense-in-depth measure.
...
3. **XPC + CoreML: standard isolation, a few integration details to confirm during implementation.** XPC services and subprocess isolation are a mature, well-understood macOS mechanism — crash isolation (helper dies, host survives) and memory isolation (separate address space and accounting) are precisely what XPC is designed for, and running CoreML inside a helper/extension process is common practice. This is *not* a feasibility unknown and does not warrant a standalone proof-of-concept. The residual items are narrow integration details to verify while building the production processor, not architecture risks: (a) **ANE access from the helper** — CoreML chooses CPU/GPU/ANE and ANE is system-mediated; confirm the helper gets ANE (worst case it falls back to GPU = slower, not broken); (b) **model cache path under sandbox** — the ~3 GB model needs a container-accessible cache directory (standard sandbox-container handling); (c) **memory under pressure on 8 GB** — see Gotcha #2; the real question is whether the OS jetsam-kills the helper mid-transcription, which is a *measurement* (handle restart-on-kill gracefully regardless). Fallback if any prove troublesome: in-process background actor.
...
16. **Custom-vocab `promptTokens` blanking bug — FIXED in argmax-oss-swift v1.1.0 (found 2026-06, Project A.5; fixed 2026-07, [PR #514](https://github.com/argmaxinc/argmax-oss-swift/pull/514)).** In v1.0.0, setting `DecodingOptions.promptTokens` for vocabulary biasing could make WhisperKit return **zero segments with no error** — a total, silent failure. Triggers: uppercase terms, certain term orderings/combinations even when lowercase. Root cause: the decode loop's early-exit checks (EOT during prefill, first-token confidence) honored throwaway tokens from the forced prompt, causing immediate segment termination. **Fixed in v1.1.0** — the EOT check now ignores prefill steps, and the first-token confidence check is anchored at the first real decoded token. The `customVocabWordMatch` AI test is re-enabled and **passes on hardware** (`make test-ai`, 2026-08-23, Apple M4). On-hardware verification (v1.1.0) confirmed: (a) mixed-case vocab works reliably — the lowercasing workaround has been removed from `VocabularyFormatter`; (b) lowercasing was in fact *degrading* accuracy: it caused a residual first-token interaction where lowercase "nasa" as the first prompt term was systematically dropped, and it forced all transcript output to lowercase (Whisper mirrors prompt casing); (c) with natural casing, recognition probabilities improved substantially (0.98-1.0 vs 0.38-0.90 lowercased); (d) **misspelled prompt terms propagate into the transcript** — our test fixture had a "gnocci" typo (correct: "gnocchi"), and with lowercase prompting the model reproduced the misspelling verbatim; with properly-cased "Gnocchi" the model produced the correct spelling. This was observed in `transcribe-cli` runs on a 63-second reference clip (sample size: 5 runs per configuration). Implication for the deferred `Vocabulary` module: user-entered terms are treated fairly literally, so typos in the custom vocabulary carry a real cost. The empty-output re-run guard (mitigation b) remains deferred — less critical now that the root cause is fixed, but still a good defense-in-depth measure.
